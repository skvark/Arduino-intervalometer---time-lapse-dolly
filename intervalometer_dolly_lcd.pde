...
// if false, sends pulse to the optocoupler which triggers the camera
bool exposing = false;

// motor speed, increments by 20
// no "zero" speed, because it can cause DC gearmotor to stall 
// -> max current passes trough, if no fuse is used the motor can suffer damage
...
int dig1Button(int pin) {
	...
	if (c>160 && c<180)
	{
		...
		e++;		// was pressed and we don't want that to happen (about 100-200 ms)
	}
...
// switch for choosing motor behavior: if on, motor moves without pauses, 
// if off motor pauses when pic is taken
...
void loop() {
	...
	if (s == 1) {
		...
		if (t == 0) {
			interval = e*1000 + b*100; // turning the display values into milliseconds,
			// max value being 9900 ms (9,9 seconds)
			pause = p*100;  // pause time, equivalent to exposure time
			divi = 10; // pulse length divider
		}
		else if (t == 1) {
			interval = e*10000 + b*1000; // full seconds, values from 0 to 99 seconds
		...
		// example: interval 2 sec, time range 0,1-9,9s, length 2000 ms / 10 = 200 ms
